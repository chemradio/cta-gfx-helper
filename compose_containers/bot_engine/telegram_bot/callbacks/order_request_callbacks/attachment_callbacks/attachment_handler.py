...
async def attachment_downloader(
    ...
    for test_stage, allowed_attachemnts in stage_attachment_map.items():
        if stage == test_stage:
            if not any(
                [
                    attachment in allowed_attachemnts
    ...
    if tg_file.file_size > config.MAX_ATTACHMENT_SIZE:
        ...
        raise AttachmentSizeExceeded(
            f"Attachment size exceeded. {tg_file.file_size} is bigger than max size {config.MAX_ATTACHMENT_SIZE}"
        )

    start_download_message = await Responder.common.wait_for_download(user_id)
